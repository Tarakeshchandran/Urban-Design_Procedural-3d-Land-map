...
public:
    ...
    Voxel generateTransition(const Voxel& voxelA, const Voxel& voxelB, TransitionType type) {
        ...
        // Default transition if no rule matches
        return createDefaultTransition(voxelA, voxelB, type);
    }
...
public:
    ...
    void initializeTestBuilding() {
        ...
        // Ground floor - Visit (Purple) - 3x3 grid
        for (int x = 0; x < 3; ++x) {
            for (int z = 0; z < 3; ++z) {
                Voxel v;
                v.type = VISIT;
        ...
        for (int level = 6; level <= 8; ++level) {
            for (int x = 0; x < 3; ++x) {
                for (int z = 0; z < 3; ++z) {
                    Voxel v;
                    v.type = LIVE;
                    v.position = Alice::vec(x * 8, 26 + (level - 6) * 4, z * 8);
                    v.dimensions = Alice::vec(8, 4, 8);
                    v.level = level;
                    v.id = nextVoxelId++;
                    buildingVoxels.push_back(v);
        ...
        cout << "- VISIT: Y = 0 to 6" << endl;
        cout << "- WORK: Y = 6 to 26" << endl;
        cout << "- LIVE: Y = 26 to 38" << endl;
