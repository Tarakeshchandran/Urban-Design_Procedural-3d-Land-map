...
#include <map>
#include <functional>
// Add this include near the top of your file with the other includes
#include <fstream>
...
public:
    ...
    void initializeTestBuilding() {
        // Default to high density mixed use
        initializeHighDensityMixedUse();
    }
    ...
    // Add this method to the ProgramTransitionSystem class in the public section:

    void exportAsOBJ(const string& filename) {
        cout << "Exporting building model to " << filename << "..." << endl;
        ...
        // Write OBJ header with info
        objFile << "# Building model exported from Urban Grammar system" << endl;
        objFile << "# Context: " << contextNames[currentContext] << endl;
        objFile << "# Edge condition: " << edgeConditionNames[currentEdge] << endl;
        objFile << "# Corner condition: " << cornerConditionNames[currentCorner] << endl;
        objFile << "# Total voxels: " << buildingVoxels.size() << endl;
        objFile << endl;

        // Vertex index counter (OBJ indices start at 1)
        int vertexCount = 1;

        // Map to store vertex indices for each voxel type
        // This will let us create different mesh groups by voxel type
        map<VoxelType, vector<int>> voxelTypeIndices;
        map<TransitionProgram, vector<int>> transitionTypeIndices;

        // Go through all voxels and create vertices for each
        for (const auto& voxel : buildingVoxels) {
            // Get corners of the voxel
            float x = voxel.position.x;
            float y = voxel.position.y;
            float z = voxel.position.z;
            float width = voxel.dimensions.x;
            float height = voxel.dimensions.y;
            float depth = voxel.dimensions.z;

            // Define 8 vertices for the cube/box
            // Format: v x y z
            objFile << "v " << x << " " << y << " " << z << endl;                             // 0: bottom-left-back
            objFile << "v " << (x + width) << " " << y << " " << z << endl;                    // 1: bottom-right-back
            objFile << "v " << (x + width) << " " << y << " " << (z + depth) << endl;            // 2: bottom-right-front
            objFile << "v " << x << " " << y << " " << (z + depth) << endl;                    // 3: bottom-left-front
            objFile << "v " << x << " " << (y + height) << " " << z << endl;                   // 4: top-left-back
            objFile << "v " << (x + width) << " " << (y + height) << " " << z << endl;           // 5: top-right-back
            objFile << "v " << (x + width) << " " << (y + height) << " " << (z + depth) << endl;   // 6: top-right-front
            objFile << "v " << x << " " << (y + height) << " " << (z + depth) << endl;           // 7: top-left-front

            // Store the starting vertex index for this voxel
            int startIdx = vertexCount;
            vertexCount += 8; // We added 8 vertices

            // Store indices based on voxel type
            if (voxel.type == TRANSITION) {
                if (transitionTypeIndices.find(voxel.program) == transitionTypeIndices.end()) {
                    transitionTypeIndices[voxel.program] = vector<int>();
                }
                transitionTypeIndices[voxel.program].push_back(startIdx);
            }
            else {
                if (voxelTypeIndices.find(voxel.type) == voxelTypeIndices.end()) {
                    voxelTypeIndices[voxel.type] = vector<int>();
                }
                voxelTypeIndices[voxel.type].push_back(startIdx);
            }
        }

        // Add a newline before faces section
        objFile << endl;

        // Define material library
        objFile << "mtllib building_materials.mtl" << endl << endl;

        // Helper function to write a face for a voxel given its starting vertex index
        auto writeFaces = [&objFile](int idx) {
            // Bottom face (0,1,2,3)
            objFile << "f " << idx << " " << (idx + 1) << " " << (idx + 2) << " " << (idx + 3) << endl;
            // Top face (4,5,6,7)
            objFile << "f " << (idx + 4) << " " << (idx + 7) << " " << (idx + 6) << " " << (idx + 5) << endl;
            // Front face (3,2,6,7)
            objFile << "f " << (idx + 3) << " " << (idx + 2) << " " << (idx + 6) << " " << (idx + 7) << endl;
            // Back face (0,1,5,4)
            objFile << "f " << idx << " " << (idx + 4) << " " << (idx + 5) << " " << (idx + 1) << endl;
            // Left face (0,3,7,4)
            objFile << "f " << idx << " " << (idx + 3) << " " << (idx + 7) << " " << (idx + 4) << endl;
            // Right face (1,2,6,5)
            objFile << "f " << (idx + 1) << " " << (idx + 5) << " " << (idx + 6) << " " << (idx + 2) << endl;
            };

        // Write voxel faces grouped by type for easier selection in 3D software

        // Write regular voxel types
        for (const auto& typeIndices : voxelTypeIndices) {
            // Set material for this voxel type
            string materialName;
            switch (typeIndices.first) {
            case VISIT: materialName = "mtl_visit"; break;
            case WORK: materialName = "mtl_work"; break;
            case LIVE: materialName = "mtl_live"; break;
            default: materialName = "mtl_default"; break;
            }

            // Create named group and set material
            objFile << "g " << getVoxelTypeName(typeIndices.first) << endl;
            objFile << "usemtl " << materialName << endl;

            // Write all voxels of this type
            for (int startIdx : typeIndices.second) {
                writeFaces(startIdx);
            }
            objFile << endl;
        }

        // Write transition voxel types
        for (const auto& typeIndices : transitionTypeIndices) {
            // Set material for this transition type
            string materialName = "mtl_transition_" + to_string(static_cast<int>(typeIndices.first));

            // Create named group and set material
            objFile << "g TRANSITION_" << getTransitionProgramName(typeIndices.first) << endl;
            objFile << "usemtl " << materialName << endl;

            // Write all transition voxels of this type
            for (int startIdx : typeIndices.second) {
                writeFaces(startIdx);
            }
            objFile << endl;
        ...
        string mtlFilename = "building_materials.mtl";
        ofstream mtlFile(mtlFilename);
        if (!mtlFile.is_open()) {
            cout << "Error: Could not open material file for writing: " << mtlFilename << endl;
            return;
        }

        // Write MTL header
        mtlFile << "# Building materials for urban grammar OBJ export" << endl << endl;

        // Define base material properties
        auto writeMaterial = [&mtlFile](const string& name, float r, float g, float b, float alpha, float specular) {
            mtlFile << "newmtl " << name << endl;
            mtlFile << "Ka 0.2 0.2 0.2" << endl; // Ambient color
            mtlFile << "Kd " << r << " " << g << " " << b << endl; // Diffuse color
            mtlFile << "Ks " << specular << " " << specular << " " << specular << endl; // Specular color
            mtlFile << "d " << alpha << endl; // Transparency
            mtlFile << "Ns 10.0" << endl; // Shininess
            mtlFile << endl;
            };

        // Write materials for base voxel types
        writeMaterial("mtl_visit", 0.8f, 0.4f, 0.8f, 0.7f, 0.3f); // Purple
        writeMaterial("mtl_work", 0.2f, 0.6f, 1.0f, 0.7f, 0.3f);  // Blue  
        writeMaterial("mtl_live", 1.0f, 0.8f, 0.2f, 0.7f, 0.3f);  // Yellow
        writeMaterial("mtl_default", 0.5f, 0.5f, 0.5f, 0.7f, 0.3f);

        // Write materials for transition programs
        writeMaterial("mtl_transition_0", 1.0f, 0.0f, 0.0f, 0.8f, 0.5f);     // SKY_LOBBY - Red
        writeMaterial("mtl_transition_1", 0.0f, 1.0f, 0.0f, 0.8f, 0.5f);     // AMENITY_FLOOR - Green
        writeMaterial("mtl_transition_2", 0.3f, 0.3f, 1.0f, 0.8f, 0.5f);     // BUFFER_ZONE - Blue
        writeMaterial("mtl_transition_3", 0.0f, 0.8f, 0.0f, 0.8f, 0.5f);     // SHARED_TERRACE - Green
        writeMaterial("mtl_transition_4", 0.5f, 0.5f, 0.5f, 0.8f, 0.5f);     // CIRCULATION_CORE - Gray
        writeMaterial("mtl_transition_5", 0.8f, 0.8f, 0.0f, 0.8f, 0.5f);     // TRANSITION_VOID - Yellow
        writeMaterial("mtl_transition_6", 0.8f, 0.4f, 0.0f, 0.8f, 0.5f);     // MEZZANINE - Orange
        writeMaterial("mtl_transition_7", 0.0f, 1.0f, 1.0f, 0.8f, 0.5f);     // SERVICE_FLOOR - Cyan
        writeMaterial("mtl_transition_8", 0.0f, 0.4f, 0.8f, 0.8f, 0.5f);     // PROGRAM_BRIDGE - Blue
        writeMaterial("mtl_transition_9", 1.0f, 0.6f, 0.0f, 0.8f, 0.5f);     // RETAIL_PODIUM - Orange
        writeMaterial("mtl_transition_10", 0.2f, 0.9f, 0.2f, 0.8f, 0.5f);    // COMMUNITY_GARDEN - Light Green
        writeMaterial("mtl_transition_11", 0.7f, 0.7f, 1.0f, 0.8f, 0.5f);    // SKY_BRIDGE - Light Blue
        writeMaterial("mtl_transition_12", 0.8f, 0.0f, 0.8f, 0.8f, 0.5f);    // TRANSIT_HUB - Magenta
        writeMaterial("mtl_transition_13", 0.9f, 0.9f, 0.3f, 0.8f, 0.5f);    // EXHIBITION_SPACE - Light Yellow
        writeMaterial("mtl_transition_14", 0.5f, 0.5f, 0.9f, 0.8f, 0.5f);    // COWORKING_PLAZA - Periwinkle
        writeMaterial("mtl_transition_15", 0.0f, 0.7f, 0.0f, 0.8f, 0.5f);    // GREEN_ROOF - Green
        writeMaterial("mtl_transition_16", 0.3f, 0.3f, 0.3f, 0.8f, 0.5f);    // PARKING_DECK - Dark Gray

        mtlFile.close();

        cout << "OBJ export complete. Files created:" << endl;
        cout << "- " << filename << " (3D geometry)" << endl;
        cout << "- " << mtlFilename << " (materials)" << endl;
    }
};
...
// In the keyPress function, add this case for exporting OBJ files when the 'o' key is pressed:
...
void keyPress(unsigned char k, int xm, int ym)
{
    if (k == 'g') {
        ...
        if (transitionSystem) {
            // Generate a filename based on context and conditions
            string filename = "building_";
            filename += to_string(static_cast<int>(currentContext)) + "_";
            filename += to_string(static_cast<int>(currentEdge)) + "_";
            filename += to_string(static_cast<int>(currentCorner)) + ".obj";

            transitionSystem->exportAsOBJ(filename);
        }
    }
}
