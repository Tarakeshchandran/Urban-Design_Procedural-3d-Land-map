...
public:
    ...
    void updatePrioritiesForContext(UrbanContext context) {
        // Update priorities based on selected context
        switch (context) {
        case HIGH_DENSITY_MIXED_USE:
            // Sky lobbies and amenities crucial
            for (auto& rule : transitionRules) {
                ...
                else if (rule.ruleName == "SharedTerraceRule") rule.currentPriority = 60;
                else if (rule.ruleName == "ServiceFloorRule") rule.currentPriority = 80;
            }
            break;
        ...
        case LOW_DENSITY_RESIDENTIAL:
            ...
            for (auto& rule : transitionRules) {
                if (rule.ruleName == "SkyLobbyRule") rule.currentPriority = 50;
                else if (rule.ruleName == "AmenityFloorRule") rule.currentPriority = 70;
        ...
        case COMMERCIAL_CORE:
            // Service and sky lobbies important
            for (auto& rule : transitionRules) {
                if (rule.ruleName == "SkyLobbyRule") rule.currentPriority = 100;
                else if (rule.ruleName == "AmenityFloorRule") rule.currentPriority = 60;
                else if (rule.ruleName == "SharedTerraceRule") rule.currentPriority = 40;
                else if (rule.ruleName == "ServiceFloorRule") rule.currentPriority = 90;
            }
            break;

        case CULTURAL_DISTRICT:
            // Balanced with emphasis on public spaces
            for (auto& rule : transitionRules) {
                if (rule.ruleName == "SkyLobbyRule") rule.currentPriority = 80;
                else if (rule.ruleName == "AmenityFloorRule") rule.currentPriority = 90;
                else if (rule.ruleName == "SharedTerraceRule") rule.currentPriority = 85;
                else if (rule.ruleName == "ServiceFloorRule") rule.currentPriority = 70;
            }
            break;

        case TRANSIT_ORIENTED:
            // Service floors and connectivity important
            for (auto& rule : transitionRules) {
                if (rule.ruleName == "SkyLobbyRule") rule.currentPriority = 85;
                else if (rule.ruleName == "AmenityFloorRule") rule.currentPriority = 75;
                else if (rule.ruleName == "SharedTerraceRule") rule.currentPriority = 65;
                else if (rule.ruleName == "ServiceFloorRule") rule.currentPriority = 100;
            }
            break;
        }

        cout << "Updated priorities for context: " << contextNames[context] << endl;
        for (const auto& rule : transitionRules) {
            cout << "  " << rule.ruleName << ": " << rule.currentPriority << endl;
        }
    }
    ...
    void initializeRules() {
        ...
        transitionRules.push_back({
            ...
            100,  // basePriority
            100   // currentPriority (will be updated based on context)
            });
...
public:
    ...
    Alice::vec getVoxelColor(const Voxel& voxel) {
        ...
        case TRANSITION:
            ...
            case SHARED_TERRACE: return Alice::vec(0.0f, 0.8f, 0.0f); // Green (changed from magenta)
